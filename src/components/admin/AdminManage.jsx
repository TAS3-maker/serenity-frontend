...
import { Plus, Pencil, Key, UserX, Trash2 } from "../../lib/icons";
...
const apiAdminToUi = (a) => ({
  ...
  twoFA: !!a.twoFA,
  lastLogin: a.lastActiveDate
    ? new Date(a.lastActiveDate).toLocaleString()
    : "—",
  joined: a.createdAt
...
export const AAdminManage = ({ showToast, currentAdminId }) => {
  ...
  const rawAdmins = adminsApi.data || [];
  const admins = rawAdmins.map(apiAdminToUi);

  const setAdmins = (updater) => {
    adminsApi.setData((prev = []) => {
      const uiPrev = prev.map(apiAdminToUi);

      const updatedUi =
        typeof updater === "function" ? updater(uiPrev) : updater;

      return updatedUi.map((a) => ({
        ...prev.find((x) => (x._id || x.id) === a.id),

        _id: a.id,
        name: a.name,
        email: a.email,

        role: a.role === "Super Admin" ? "superadmin" : "admin",
  ...
  const mutate = (id, patch) =>
    setAdmins((as) => as.map((a) => (a.id === id ? { ...a, ...patch } : a)));
  ...
  const saveEdit = async () => {
    ...
    try {
      if (target) {
        ...
        const password = form.email.split("@")[0] + "Temp1!";
  ...
  const COLS = [
      ...
      render: (_, a) => (
        <div className="flex items-center gap-3">
          <AvatarUpload
            initials={a.avatar}
            size={36}
            gradient={`linear-gradient(135deg,${C.teal},#1E7145)`}
          />
...

    {
      ...
      label: "Role",
      render: (v) => (
        <Badge label={v} variant={ROLE_COLOR[v] || "grey"} />
      ),
    },
...

    {
      ...
      label: "Last Login",
      render: (v) => (
        <span className="text-[12px] text-[var(--textMuted)]">{v}</span>
      ),
    },
          ...
          {a.id !== currentId && (
            <>
              <Button
                ...
                onClick={async () => {
                  const newStatus =
                    a.status === "active" ? false : true;

                  try {
                    await api.admin.update(a.id, {
                      isActive: newStatus,
                    });
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        ...
        <Button icon={Plus} onClick={() => openEdit(null)}>
          Add Admin
        </Button>
      </div>
      ...
      <Card noPad>
        <Table
          cols={COLS}
          rows={admins.slice((page - 1) * PER, page * PER)}
        />
      ...
      <Modal
        ...
        title={target ? "Edit Admin" : "Add Admin"}
      >
        <div className="grid grid-cols-2 gap-3">
          ...
          <Select
            label="Timezone"
            value={form.timezone}
            onChange={(e) => sf("timezone", e.target.value)}
          >
            {[
              "UTC",
              "America/New_York",
      ...
      <Modal
        ...
        title={`Change Password — ${target?.name}`}
        width={440}
      >
        <div
          className="p-3 rounded-xl mb-4 text-[13px] font-medium text-[var(--teal)]"
          style={{
        ...
        <div className="flex gap-2.5 mt-2">
          ...
          <Button
            className="flex-1"
            disabled={
              pw.newPw.length < 8 || pw.newPw !== pw.confirm
            }
            onClick={async () => {
              try {
                await api.admin.resetPassword(
                  target.id,
                  pw.newPw
                );
      ...
      <Confirm
        ...
        onConfirm={async () => {
          try {
            await api.admin.delete(delId);

            setAdmins((as) =>
              as.filter((a) => a.id !== delId)
            );
